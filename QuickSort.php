...
Choose a Pivot: Select a pivot element from the array. The pivot can be chosen in various ways, such as the first element, 
last element, or a random element.Partitioning: Rearrange the elements in the array such that all elements less than the pivot are
placed before it, and all elements greater than the pivot are placed after it. After partitioning, the pivot element is in its 
final sorted position.Recursively Sort Subarrays: Recursively apply the Quick Sort algorithm to the subarrays formed by partitioning
the original array. One subarray contains elements less than the pivot, and the other contains elements greater than the pivot.
